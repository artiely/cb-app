        ...
        : item.serverSkus ? item.serverSkus.length : 0)>1" class="cb-card-ctrl-choice tap-area-sku" @click="showSku(item,index)">选规格
                  ...
                  <div v-else>
                    <div class="plus-cut-ctrl-box" v-if="item.goodsSkus&&item.goodsSkus[0].count>0">
                      <div class="ctrl-cut tap-area" @click="cut({sku:item.goodsSkus[0]})">-</div>
                      <div type="tel" class="ctrl-input" maxlength="7">{{item.goodsSkus[0].count}}</div>
                      <div class="ctrl-plus tap-area" @click="add({goods:item,sku:item.goodsSkus[0],gasType:'1'})">+</div>
                    </div>
                    <div v-else class="plus-cut-ctrl-add">
                      <div class="ctrl-plus ctrl-icon tap-area" style="background:#1690ff;color:#fff" @click="add({goods:item,sku:item.goodsSkus[0],gasType:'1'})">+</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="right-select-sku">
                ...
                <div >
                  <div class="plus-cut-ctrl-box" v-if="it.count>0">
                    <div class="ctrl-cut ctrl-icon tap-area" @click="cut({sku:it})">-</div>
                    <div type="tel" class="ctrl-input" maxlength="7">{{it.count}}</div>
                    <div class="ctrl-plus ctrl-icon tap-area" @click="add({goods:currGoods,sku:it,gasType:'1'})">+</div>
                  </div>
                  <div v-else class="plus-cut-ctrl-add">
                    <div class="ctrl-plus ctrl-icon tap-area" @click="add({goods:currGoods,sku:it,gasType:'1'})">+</div>
                  </div>
