<template>
  <div class="RoleManageEditBox">
    <v-header title="编辑角色" :border="true" :isBack="true" righttext="保存" @right-click="save">
    </v-header>
...
export default {
  ...
  methods: {
    ...
    async allAuthListFunc() {
      ...
      let res = await this.$api.ROLE_MENU()
      if (res.status === 1) {
    ...
    deleteFunc() {
      ...
      that.$dialog
        ...
        .then(async() => {
          // on confirm
          let res = await that.$api.ROLE_DELETE({ id: that.query.id })
          // console.log('删除中打印的res =', res)
          if (res.status === 1) {
            that.$toast('删除成功。')
            that.$router.push({
              name: 'RoleManagement'
