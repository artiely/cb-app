    ...
    <div class="wrapper-goods">
      ...
      <div class="right-list">
        ...
        : item.serverSkus ? item.serverSkus.length : 0)>1" class="cb-card-ctrl-choice tap-area" @click="showSku(item,index)">选规格
                  ...
                  <div v-else>
                    <div class="ctrl-nolimit tap-area" @click="noLimit(item,'goods','2')" :class="{'no-limit-active':item.serverSkus[0].count==-1}">不限次</div>
                    <div class="plus-cut-ctrl-box tap-area" v-if="item.serverSkus&&item.serverSkus[0].count>0">
                      <div class="ctrl-cut tap-area" @click="cut(item,'goods','2')">-</div>
                      <div type="tel" class="ctrl-input" maxlength="7">{{item.serverSkus[0].count}}</div>
                      <div class="ctrl-plus tap-area" @click="add(item,'goods','2')">+</div>
                    </div>
                    <div v-else class="plus-cut-ctrl-add">
                      <div class="ctrl-plus ctrl-icon tap-area" @click="push(item,'goods','2')">+</div>
                    </div>
                  </div>
                </div>
                <!-- 折扣卡控件 -->
        ...
        : item.serverSkus ? item.serverSkus.length : 0)>1" class="cb-card-ctrl-choice tap-area">选规格
                    <v-dot :number="item.count" /> </span>
                  <v-icon class="tap-area" v-else :name="item.serverSkus[0].checked?'icon-queding1':'icon-yuan'"></v-icon>
                </div>
              </div>
            </li>
            <!-- 单个规格e -->
      ...
      <div class="sku-goods-wrapper">
        <cube-scroll ref="skuScroll">
          <div class="sku-goods-scroll-inner">
            <div class="sku-goods-item " v-for="(it,i) in skuData" :key="it.id">
              ...
              <div class="right-select-sku">
                <!-- 次卡 -->
                <div v-if="cardType=='3'">
                  <div class="ctrl-nolimit tap-area" @click="noLimit(it,'sku','2')" :class="{'no-limit-active':it.count==-1}">不限次</div>
                  <div class="plus-cut-ctrl-box" v-if="it.count>0">
                    <div class="ctrl-cut ctrl-icon tap-area" @click="cut(it,'sku','2')">-</div>
                    <div type="tel" class="ctrl-input" maxlength="7">{{it.count}}</div>
                    <div class="ctrl-plus ctrl-icon tap-area" @click="add(it,'sku','2')">+</div>
                  </div>
                  <div v-else class="plus-cut-ctrl-add">
                    <div class="ctrl-plus ctrl-icon tap-area" @click="push(it,'sku','2')">+</div>
                  </div>
                </div>
                <!-- 折扣卡 -->
